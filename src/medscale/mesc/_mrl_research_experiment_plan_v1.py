...
class PlanTierAllowance:
    ...
    def __post_init__(self) -> None:
        ...
        if self.tier not in (EvaluationTier.SEARCH, EvaluationTier.REPLICATION) and self.max_queries:
            raise ResearchExperimentPlanError(
        ...
        if self.tier in (EvaluationTier.SEALED, EvaluationTier.EXTERNAL_ASSURANCE) and (
            self.max_result_exposures or self.allowed_result_fields
        ):
            raise ResearchExperimentPlanError(
...
class ResearchExperimentPlan:
    ...
    def _validated_snapshot(self) -> ResearchExperimentPlan:
        ...
        _require_exact_instance(self, ResearchExperimentPlan, "research_experiment_plan")
...
def _validate_plan(plan: ResearchExperimentPlan) -> None:
    _require_kebab_id(plan.experiment_plan_id, "experiment_plan_id", pattern=_PLAN_ID)
    ...
    _require_sorted_unique_text(plan.mutation_surfaces, "mutation_surfaces", allow_empty=True)
    for surface in plan.mutation_surfaces:
        _require_plan_mutation_surface(surface, objective.allowed_mutation_surfaces)
    ...
    _require_exact_instances(plan.evaluator_identities, EvaluatorIdentity, "evaluator_identities")
    ...
    for identity in plan.evaluator_identities:
        if not set(identity.tiers).issubset(set(plan.evaluation_tiers)):
            raise ResearchExperimentPlanError(
    ...
    _require_exact_instances(plan.tier_allowances, PlanTierAllowance, "tier_allowances")
    ...
    objective_exposure = {
        policy.tier: policy for policy in objective.tier_result_exposure_policy
    }
    for allowance in plan.tier_allowances:
        _require_tier_allowance_within_objective(allowance, objective, objective_exposure)
...
def _snapshot_expected_manifest(
    ...
    _require_exact_instance(value, ExpectedExperimentManifestBinding, "expected_manifest")
...
def _snapshot_evaluator_identity(value: EvaluatorIdentity) -> EvaluatorIdentity:
    ...
    try:
        ...
        raise ResearchExperimentPlanError("evaluator identity failed canonical revalidation") from exc


def _require_resource_subset(plan: ResourceBudget, objective: ResourceBudget) -> None:
    fields = (
        "wall_clock_seconds",
        "compute_seconds",
        "input_tokens",
...
def _require_plan_mutation_surface(surface: str, allowed: tuple[str, ...]) -> None:
    _require_canonical_relative_path(surface, "mutation_surfaces")
    for envelope in allowed:
        if surface == envelope or (envelope.endswith("/") and surface.startswith(envelope)):
            return
    raise ResearchExperimentPlanError(
...
def _require_exact_instances(values: tuple[object, ...], expected: type[object], label: str) -> None:
    if type(values) is not tuple:
        raise ResearchExperimentPlanError(f"{label} must be an exact tuple")
    for value in values:
...
def _require_exact_enum(value: enum.Enum, expected: type[enum.Enum], label: str) -> None:
    if type(value) is not expected:
        raise ResearchExperimentPlanError(f"{label} must be an exact {expected.__name__} value")


def _require_kebab_id(value: str, label: str, *, pattern: re.Pattern[str]) -> None:
    _require_text(value, label)
