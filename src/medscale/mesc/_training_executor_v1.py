...
This module independently revalidates canonical training authority, snapshots one exact
execution manifest, and invokes only an explicitly injected backend. The core performs no
model loading, network access, provider access, GPU work, or training by itself.
"""
...
class TrainingExecutionError(ValueError):
    """Raised when training execution cannot cross the canonical executor boundary."""


@dataclass(frozen=True, slots=True)
class TrainingExecutionEnvironment:
    """Caller-observed environment identity that must equal the selected run plan."""
    ...
    def environment_sha256(self) -> str:
        """Return the deterministic identity of the observed execution environment."""
...
class TrainingResultArtifact:
    """One content-addressed canonical result artifact emitted by a backend."""
        ...
        if self.disposition == "SUCCEEDED":
            ...
            if self.artifacts:
                raise TrainingExecutionError(
                    "FAILED or ABORTED backend result cannot claim canonical result artifacts"
                )
...
class TrainingExecutionManifest:
    """Core-owned immutable input supplied to exactly one training backend invocation."""
    ...
    def execution_manifest_sha256(self) -> str:
        """Return the deterministic identity of this exact backend invocation manifest."""
        return content_hash(self.to_dict())

    def to_dict(self) -> dict[str, object]:
        """Return the canonical backend manifest payload."""
        return {
...
class TrainingBackend(Protocol):
    """Injected execution boundary; default tests provide fake backends only."""
...
class TrainingExecutionReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if self.result_manifest_sha256 is not None:
            _require_sha256(self.result_manifest_sha256, field="result_manifest_sha256")
        _require_git_sha(self.revision, field="revision")
        _require_git_sha(self.repository_sha, field="repository_sha")
        _require_git_sha(self.repository_tree, field="repository_tree")
        ...
        if finished < started:
            raise TrainingExecutionError("finished_at must not precede started_at")
        if not isinstance(self.result_artifacts, tuple):
            raise TrainingExecutionError("result_artifacts must be an immutable tuple")
        if any(type(item) is not TrainingResultArtifact for item in self.result_artifacts):
            raise TrainingExecutionError(
        ...
        if self.disposition == "SUCCEEDED":
            if not self.result_artifacts or self.result_manifest_sha256 is None:
                raise TrainingExecutionError(
                    "SUCCEEDED receipt requires result artifacts and result manifest identity"
                )
            if self.failure_reason is not None:
            ...
            if self.result_artifacts or self.result_manifest_sha256 is not None:
                raise TrainingExecutionError(
                    "FAILED or ABORTED receipt cannot claim canonical result artifacts"
                )
            _require_text(self.failure_reason, field="failure_reason")

    @property
    def receipt_sha256(self) -> str:
        """Return the deterministic identity of this execution receipt."""
        return content_hash(self.to_dict())

    def to_dict(self) -> dict[str, object]:
        """Return the canonical execution-receipt payload."""
        return {
...
def execute_training(
    *,
    ...
    """Validate all canonical authority and invoke one explicitly supplied backend."""
    _require_exact_input(manifest, TrainingReadinessManifest, field="manifest")
    _require_exact_input(readiness, TrainingReadinessReport, field="readiness")
    _require_exact_input(launch_plan, TrainingLaunchPlan, field="launch_plan")
    _require_exact_input(corpus_binding, TrainingCorpusBindingReport, field="corpus_binding")
    _require_exact_input(
        local_assets,
        TrainingLocalAssetAttestationReport,
        field="local_assets",
    )
    _require_exact_input(environment, TrainingExecutionEnvironment, field="environment")
    ...
    try:
        recomputed_readiness = assess_training_readiness(manifest)
        if readiness != recomputed_readiness:
            raise TrainingExecutionError(
                "supplied readiness report does not match recomputed readiness"
            )
        if not recomputed_readiness.can_launch_training:
            raise TrainingExecutionError("recomputed readiness is not READY_TO_LAUNCH")
        rebuilt_launch = build_training_launch_plan(
            manifest=manifest,
            readiness=recomputed_readiness,
    ...
    manifest_sha256 = execution_manifest.execution_manifest_sha256
    ...
    if execution_manifest.execution_manifest_sha256 != manifest_sha256:
...
def _require_corpus_binding(
    ...
    *,
    ...
    if not binding.can_attest_local_artifact or binding.disposition != "PASS":
...
def _require_local_attestation(
    ...
    *,
    ...
    expected = (
        ...
        ("launch_plan_sha256", attestation.launch_plan_sha256, launch_plan.plan_sha256),
        ("run_plan_sha256", attestation.run_plan_sha256, run_plan.run_plan_sha256),
        (
        ...
        ("expected_weights_sha256", attestation.expected_weights_sha256, run_plan.weights_sha256),
        ("observed_weights_sha256", attestation.observed_weights_sha256, run_plan.weights_sha256),
        (
...
def _snapshot_backend_result(result: TrainingBackendResult) -> TrainingBackendResult:
    try:
        artifacts = tuple(
...
def _require_exact_input(value: object, expected_type: type[object], *, field: str) -> None:
