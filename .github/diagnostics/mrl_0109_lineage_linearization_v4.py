...
class_end = text.index("\n\ndel _bind_learning_admission_trust", class_start)
...
helper_start = text.index("def _require_acyclic_parent_lineage(")
helper_end = text.index("\ndef _require_source_permission_binding", helper_start)
new_helpers = '''def _parent_graph_postorder(
...
launder_start = text.index("def _require_no_lineage_laundering(")
launder_end = text.index("\ndef _require_transformation_lineage", launder_start)
text = text[:launder_start] + text[launder_end + 1 :]

parents_start = text.index("def _require_parent_refs(")
parents_end = text.index("\ndef _require_learning_surfaces", parents_start)
text = text[:parents_start] + text[parents_end + 1 :]
