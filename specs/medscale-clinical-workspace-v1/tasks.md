...
**Implemented surfaces (audit):** each audit event is one immutable object whose identity is derived from its digest, chained to its predecessor's digest from a genesis digest. The chain is contiguous, replay of a stored event collides instead of appending, and deletion of content plus recording of that deletion is one store transaction so audit metadata survives while the content does not. The caller supplies the occurrence time; the package imports no clock.
