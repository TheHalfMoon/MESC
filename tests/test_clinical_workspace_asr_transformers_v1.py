...
from medscale_workspace import asr as asr_mod  # noqa: E402
from medscale_workspace.asr import AsrStatus  # noqa: E402
...
def test_manifest_mismatch_blocks_loading(tmp_path: Path) -> None:
    ...
    with pytest.raises(AsrManifestError):
        asr_mod.verify_local_snapshot(
            snapshot, drifted_manifest("model_id", "openai/whisper-large-v3")
        )
    with pytest.raises(AsrManifestError):
...
def test_loader_calls_are_local_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert len(calls) == 2
    for _kind, name, kwargs in calls:
...
def test_real_backend_output_passes_same_validation(
...
def test_real_path_rejects_malformed_backend_output(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
