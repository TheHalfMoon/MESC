...
def test_valid_replay_receipt_identity_mutation_fails_closed() -> None:
    procedure = _candidate_procedure()
    cases = list(_cases())
    object.__setattr__(cases[0].replay_receipt, "candidate_sha256", "f" * 64)
...
def test_valid_nested_case_identity_mutation_fails_closed() -> None:
    procedure = _candidate_procedure()
    cases = _cases()
    report = build_procedure_transfer_test_report(procedure, cases)
    object.__setattr__(cases[0], "evidence_artifact_sha256", "f" * 64)

    with pytest.raises(ProcedureTransferTestError, match="identity changed"):
        cases[0].semantic_dict()
    with pytest.raises(ProcedureTransferTestError, match="identity changed"):
        _ = report.content_sha256


def test_valid_transfer_report_identity_mutation_fails_closed() -> None:
    report = build_procedure_transfer_test_report(_candidate_procedure(), _cases())
