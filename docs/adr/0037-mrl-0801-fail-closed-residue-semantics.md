...
The current hardened implementation therefore uses `O_TMPFILE` plus `linkat(..., AT_EMPTY_PATH)` for atomic no-replace publication and performs no automatic identity-check-then-unlink cleanup. This safely prevents overwrite and foreign-entry deletion, but its current capability proof publishes and retains a witness inside the transaction directory and then returns `BLOCKED` before remote metadata access. That safe V1 substrate is not operational and does not by itself satisfy Issue #389.
...
This is an expensive security and evidence-contract decision under Rule R6. Implementation of the decision below is prohibited until this ADR is explicitly accepted.
...
- Issue #389 acceptance text must be amended after, and only after, explicit ADR acceptance.

## Alternatives considered

- **Keep the current always-blocked witness V1** — Safe but rejected as the long-term operational design because it cannot satisfy Issue #389 or produce genuine MRL-0801 acquisition evidence.
...
Until this ADR is explicitly accepted:

- PR #390 must not be merged as closing Issue #389 solely on the basis of the current always-blocked V1 substrate.
- The existing safe fail-closed code may be qualified and reviewed, but operational activation and Issue #389 acceptance amendment remain unauthorized.

If accepted, implementation must add deterministic network-free coverage proving at least:
...
Rule R6 remains controlling: **propose → explicit approval → implement**.
